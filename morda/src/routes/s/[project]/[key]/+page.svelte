...
{:else}
  ...
  <div class="dock" bind:this={dockEl} style="--pane: {paneW}px">
    ...
    <div class="dock-asks" class:sheet-open={sheet}>
      ...
      {/if}
      {#if data.pending_hitl && !cliDialog}
        <!-- родная форма AskUserQuestion ждёт человека В ОКНЕ ПРИЛОЖЕНИЯ:
             морда её показывает, но кликнуть вариант можно только там.
             Когда та же форма стоит на экране CLI, показываем ТОЛЬКО её —
             она кликабельная, а эта нет: два одинаковых блока подряд сбивали
             с толку (CTO 21.08: «нахуй тут два раза вылезло») -->
        <article class="hitl">
