...
export function resumeForInput({ project, key, text }) {
  ...
  // Тикет не переназначаем текстом сообщения. Порядок силы:
  // 1) тикет записи реестра — его задал человек в форме запуска, и
  //    registryRecord принял бы любой переданный ключ СИЛЬНЕЕ него;
  // 2) заголовок исходной сессии — для усыновления ЧУЖОЙ (не раннерской)
  //    сессии, у которой записи ещё нет, а тикет уже свой;
  // 3) текст — последний фолбэк, когда молчат оба. Без него усыновлённая
  //    сессия вставала «вне эпиков»: /api/say шлёт текст без поля ticket.
  return runnerStart({ project, name, goal: text, resumeId: key,
    ticket: reg.sessions[name]?.ticket ?? ticketOf({ goal: meta.title })
      ?? ticketOf({ goal: text }),
