...
<TooltipPortal {...portalProps}>
	<TooltipPrimitive.Content
		...
		{side}
		class={cn(
			// Подсказка — приподнятая поверхность (--bg-3), а не инверсия в
			// светлое: инверсия в тёмном интерфейсе бьёт по глазам и читается
			// как чужой элемент. Границы нет — её роль играет сама ступень.
	...
	>
		...
		<TooltipPrimitive.Arrow>
			{#snippet child({ props })}
				<div
					class={cn(
						"size-2.5 translate-y-[calc(-50%-2px)] rotate-45 rounded-[2px] z-50 bg-accent fill-accent",
