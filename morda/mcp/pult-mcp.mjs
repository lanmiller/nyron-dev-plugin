...
const tools = {
  ...
  pult_start: {
    description:
      'Поднять новую CLI-сессию через раннер пульта — с гейтом паспорта, судьёй и автопинками. Проект на vibe-pult — рабочая сессия со скиллом work, одна на задачу. Проект на nyron-dev — для эпика ОДИН диспетчер со скиллом nyron-waves, волны дальше плодит он. Слот подписки задавай, когда нужен параллельный лимит (Мариха, stovpe3tt).',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    if (method === 'initialize') {
      send({ jsonrpc: '2.0', id, result: {
        ...
        instructions: [
          ...
          '  ИСКЛЮЧЕНИЕ — прямая просьба человека делать САМОМУ («повайбим',
          '  здесь», «сделай сама», «не поднимай сессию»): тогда ты исполнитель',
          '  в этой сессии (скилл work на vibe-pult, nyron-dev:vibe на',
          '  nyron-dev), пульт не трогаешь. Оркестратор — дефолт, не запрет.',
          '  Проект на vibe-pult (.claude/vibe-pult.md): рабочая сессия со',
          '  скиллом work, одна на задачу, mode:"bypass". Проект на nyron-dev:',
          '  эпик — ОДИН диспетчер со скиллом nyron-waves (mode:"bypass",',
          '  mcp:"strict"); волны дальше плодит он сам /goal-чипами.',
          '- «Повайбим» / поправить на горячую (вёрстка, карточки, ленд) —',
          '  НЕ волна: подними ОДНУ сессию vibe-<тема> со скиллом',
          '  nyron-dev:vibe (mode:"bypass", mcp пусто — ей нужен стенд).',
          '  Правки диктуй ей pult_send-ом мид-тёрн; финал — команда',
          '  «закругляемся»: она сгрузит зонтик-тикет в Jira и запустит',
          '  кросс-ревью фоном сама. Тесты и волны в вайбе не разворачивать.',
          '- Ревью — КРЕСТ-НАКРЕСТ по автору: писал Claude → Astra (codex),',
          '  писал codex → Fable (claude); cross-review.sh выбирает сам (-e auto).',
          ...
          '- ДОВЕДЕНИЕ ДО КОНЦА: между твоими ходами конвейер ведёт ДЕЖУРНЫЙ',
          '  (постоянная сессия «duty» пульта — разбирает вопросы, пинает',
          '  застрявших; ты чат и исполняешься только в момент хода). На',
          '  СВОЁМ ходе всё равно сверяйся с pult_fleet (кто жив/застрял),',
          '  спорную сессию смотри pult_screen и суди pult_judge. Финал — по',
          '  факту в pult_fleet/Jira, не по обещанию.',
