...
7. ПЕРЕУСЛОЖНЕНИЕ (код, которого могло не быть): лишний слой/обёртка без
   ...
   зависимости. Каждое — с пруфом: чем конкретно заменяется. ОБРАТНОЕ тоже
   замечание: НЕ предлагай срезать то, за что проект платит кодом осознанно
   (компоненты дизайн-системы, флаги фич, тесты, проверки доступа, обработка
   ошибок) — «меньше строк» само по себе не аргумент.
