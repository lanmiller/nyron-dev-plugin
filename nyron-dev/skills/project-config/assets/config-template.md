...
merge_rights: dispatcher     # кто сливает в main — лестница режимов:
                             # wave — волны мержат и пушат сами (одиночка, нет
                             #   живого стенда на main);
                             # dispatcher (дефолт) — волны отдают MR, сливает
                             #   диспетчер по очереди;
                             # lead | cto — сводит и готовит диспетчер, право
                             #   слить только у названного человека

max_parallel_waves: 4        # сколько сессий-волн запускать ОДНОВРЕМЕННО: потолок
                             # машины и лимитов подписки. Диспетчер режет эпик
                             # на максимум параллели и упирается в это число.
...
quality_floor:               # СТОП-СТУПЕНИ: что в этом проекте минимизации НЕ подлежит.
                             ...
                             # без этого списка она срезает нужное — ровно та критика,
                             # за которую разнесли ponytail-бенчмарк: метрика LOC против
                             # надёжности. Только для кодовых проектов (type: development).
  - "компонент дизайн-системы вместо голого HTML"
  - "новая фича — за флагом"
  - "тест с описанием и регистрацией в каталоге"
  - "проверки доступа — через канонический стор/сервис прав, не инлайном"

flows: []                    # РЕАЛЬНЫЕ режимы/флоу продукта, трогающие общие данные
                             # (например: create, restart/переиспользование, edit,
                             # publish→use). Эту карту получают Astra-ревьюер и
...
roles:                       # оверрайды ролевых карточек team-roles (все опциональны)
  ...
  # Реестр внешних участников — тоже записи roles. У кого plugin: false —
  # НЕТ будки, hub_post, скиллов и MCP плагина. Скилл, составляющий промт
  # человеку, ОБЯЗАН свериться с реестром: участнику без плагина — никаких
  # hub_post и скиллов в инструкциях, отчёт — по report_channel.
  helper-name:                 # слаг участника (пример)
    plugin: false              # «без плагина»: работает в своём Клоде/чате
    report_channel: "чат и коммит документов"   # канал отчёта вместо будки
    notes: "кто это и что делает — одной строкой"
